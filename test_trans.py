...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct  3 01:28:29 2018
...
def comp_accu(tpy, ty):
    #import numpy as np
    #print(np.argmax(tpy, axis = 1))
    #print(np.argmax(ty, axis = 1))
...
cu_accu = 0
while True:
    m.step_train(max_iter = 10, iter_graph = 0 , iter_inst = 1, iter_label = 0)   # perform a training step
    _, _, tpy = m.predict(tx)                                                         # predict the dev set
    #print(np.argmax(tpy, axis = 1))
    #print( np.argmax(ty, axis = 1))
